...
class Telnet {
	...
	public function __construct($host = '127.0.0.1', $port = '23', $timeout = 10, $prompt = '$', $stream_timeout = 1) {
		...
		$this->TERM_TYPE = chr(24);
		$this->ECHO = chr(1);
		$this->GO_AHEAD = chr(3);
		$this->TELNET_NAWS = chr(31);
...
//error_log( 'I DONT ' . ord($opt));
				...
				$opt = $this->getc();
				if($opt == $this->TERM_TYPE) {
					$cmd_list = [$this->SEND, $this->IAC, $this->SE];
					$receive = [];
					for($i = 0;$i < count($cmd_list); $i++) {
						$receive[] = $this->getc();
					}
					fwrite($this->socket, $this->IAC . $this->SB . $this->TERM_TYPE . $this->IS . 'VT100' . $this->IAC . $this->SE);
				} else {
					do {
						$c = $this->getc();
						error_log(ord($c) . ' ');
